...
Zenoh.Native provides P/Invoke bindings to the Zenoh C library, allowing .NET applications to use Zenoh's distributed messaging capabilities. The native Zenoh runtime is embedded in the NuGet package, so no separate installation is required.
...
```bash
dotnet add package Zenoh.Native
```
